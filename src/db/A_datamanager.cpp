...
Pelicula DataManager::obtenerPelicula(int id) {
  ...
  if (rc != SQLITE_OK) {
    std::cerr << "Error al enlazar parámetro: " << sqlite3_errmsg(db)
              << std::endl;
...
std::vector<Reserva> DataManager::obtenerReservasDeSesion(int idSesion) {
  const char* query =
      "SELECT id, sesion_id, fila, columna FROM reservas WHERE sesion_id = ?";
  sqlite3_stmt* stmt;
...
Sesion DataManager::obtenerSesion(int id) {
  const char* query =
      "SELECT pelicula_id, sala_id, strftime('%s', fecha_hora) FROM sesiones "
      "WHERE id = ?";
  sqlite3_stmt* stmt;
...
std::vector<Sala> DataManager::obtenerSalasDeCine(int idCine) {
  const char* query =
      "SELECT id, cine_id, numero_sala, filas, columnas FROM salas WHERE "
      "cine_id = ?";
  sqlite3_stmt* stmt;
...
std::vector<Sesion> DataManager::obtenerSesiones() {
  const char* query =
      "SELECT id, pelicula_id, sala_id, strftime('%s', fecha_hora) FROM "
      "sesiones";
  sqlite3_stmt* stmt;
...
bool DataManager::crearCine(const Cine& cine) {
  ...
  rc = sqlite3_bind_text(stmt, 1, cine.getNombre().c_str(), -1,
                         SQLITE_TRANSIENT);
  if (rc != SQLITE_OK) {
    std::cerr << "Error al enlazar parámetro: " << sqlite3_errmsg(db)
              << std::endl;
  ...
  rc = sqlite3_bind_text(stmt, 2, cine.getDireccion().c_str(), -1,
                         SQLITE_TRANSIENT);
  if (rc != SQLITE_OK) {
    std::cerr << "Error al enlazar parámetro: " << sqlite3_errmsg(db)
              << std::endl;
...
bool DataManager::crearSala(const Sala& sala) {
  const char* query =
      "INSERT INTO salas (cine_id, numero_sala, filas, columnas) VALUES (?, ?, "
      "?, ?)";
  sqlite3_stmt* stmt;
...
bool DataManager::crearPelicula(const Pelicula& pelicula) {
  ...
  rc = sqlite3_bind_text(stmt, 1, pelicula.getTitulo().c_str(), -1,
                         SQLITE_TRANSIENT);
  if (rc != SQLITE_OK) {
    std::cerr << "Error al enlazar parámetro: " << sqlite3_errmsg(db)
              << std::endl;
  ...
  rc = sqlite3_bind_text(stmt, 2, generoToString(pelicula.getGenero()).c_str(),
                         -1, SQLITE_TRANSIENT);
  if (rc != SQLITE_OK) {
    std::cerr << "Error al enlazar parámetro: " << sqlite3_errmsg(db)
              << std::endl;
...
bool DataManager::crearSesion(const Sesion& sesion) {
  const char* query =
      "INSERT INTO sesiones (pelicula_id, sala_id, fecha_hora, precio_entrada) "
      "VALUES (?, ?, datetime(?, 'unixepoch'), 10.0)";
  sqlite3_stmt* stmt;
...
bool DataManager::actualizarSala(int id, const Sala& sala) {
  const char* query =
      "UPDATE salas SET cine_id = ?, numero_sala = ?, filas = ?, columnas = ? "
      "WHERE id = ?";
  sqlite3_stmt* stmt;
...
bool DataManager::actualizarSesion(int id, const Sesion& sesion) {
  const char* query =
      "UPDATE sesiones SET pelicula_id = ?, sala_id = ?, fecha_hora = "
      "datetime(?, 'unixepoch') WHERE id = ?";
  sqlite3_stmt* stmt;
...
bool DataManager::actualizarReserva(int id, const Reserva& reserva) {
  const char* query =
      "UPDATE reservas SET sesion_id = ?, fila = ?, columna = ? WHERE id = ?";
  sqlite3_stmt* stmt;
