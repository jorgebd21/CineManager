...
class ExpirationToctouTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    (void)SqliteStatement(db.getDb(), "INSERT INTO cines (id, nombre) VALUES (1, 'Cine TOCTOU');").step();
    (void)SqliteStatement(db.getDb(), "INSERT INTO peliculas (id, titulo, genero, duracion) VALUES (1, 'Expiration Movie', 'DRAMA', 100);").step();
    (void)SqliteStatement(db.getDb(), "INSERT INTO salas (id, cine_id, numero_sala, filas, columnas) VALUES (1, 1, 1, 5, 5);").step();
    (void)SqliteStatement(db.getDb(), "INSERT INTO sesiones (id, pelicula_id, sala_id, fecha_hora, precio_entrada) VALUES (1, 1, 1, datetime('now', '+2 hours'), 8.0);").step();
  }
};
