    ...
    switch (opcion) {
      ...
      case 2: {
        ...
        for (auto& sesion : sesiones) {
          ...
          cout << "Hora: "
               << std::put_time(std::localtime(&hora), "%Y-%m-%d %H:%M:%S")
               << endl;
        ...
        Sala sala_seleccionada =
            db.obtenerSala(db.obtenerSesion(sesion_id).getIdSala());
        ...
        for (const auto& r : reservas) {
          if (r.getFila() == reserva_nueva.getFila() &&
              r.getColumna() == reserva_nueva.getColumna()) {
            ya_ocupado = true;
            break;
          }
        }
      ...
      case 3:
        if (reserva_actual.getFila() == -1) {
          cout << "Debe selecionar una sesión y un asiento para poder continuar"
               << endl;
          break;
        }
        ...
        cout << "Asiento: " << reserva_actual.getFila() + 1 << ", "
             << reserva_actual.getColumna() + 1 << endl;
        break;
      case 4:
